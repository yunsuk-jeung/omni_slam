...
bool StereoVO::setup(const std::string& config_path) {
  ...
  sliding_window_->set_max_size(SVOConfig::max_keyframe_size + 1u);
  optical_flow_ =
...
void StereoVO::process(std::shared_ptr<Frame>& frame) {
  if (!frame || !frame->get_tracking_result_ptr()) {
    return;
  }
...
void StereoVO::track(std::shared_ptr<Frame>& frame) {
  ...
  std::shared_ptr<Frame> latest_frame = sliding_window_->get_frame(latest_id);
  if (latest_frame) {
  ...
  while (kf_ids.size() > SVOConfig::max_keyframe_size) {
    ...
    if (!selected) {
      ...
      for (auto it1 = kf_ids.begin(); it1 != end_minus_2; ++it1) {
        ...
        std::shared_ptr<Frame> last_kf = sliding_window_->get_frame(last_kf_id);
        if (!last_kf) {
          continue;
        }
        double score = std::sqrt((frame_i->get_twb().translation()
