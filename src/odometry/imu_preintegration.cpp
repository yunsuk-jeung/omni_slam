...
void ImuPreintegration::PropagateError(const Eigen::Matrix3d& R_start,
  ...
  F.block<3, 3>(6, 3) =
    -0.5 * R_start * a0_hat * dt_sec - 0.5 * R_next * a1_hat * tmp * dt_sec;
  F.block<3, 3>(6, 9)  = -0.5 * (R_start + R_next) * dt_sec;
  F.block<3, 3>(6, 12) = 0.5 * R_next * a1_hat * dt2;

  F.block<3, 3>(0, 3) =
    -0.25 * R_start * a0_hat * dt2 - 0.25 * R_next * a1_hat * tmp * dt2;
...
ImuPreintegration::Matrix15d ImuPreintegration::GetInformation(double damping) const {
  ...
  Eigen::LDLT<Matrix15d> ldlt(covariance_regularized);
  if (ldlt.info() != Eigen::Success) {
    return Matrix15d::Zero();
  }
