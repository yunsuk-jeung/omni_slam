...
int main(int argc, char** argv) {
  LogI("Starting SLAM application");
  ...
  if (!stereo_vio.setup(config_path.string())) {
    LogE("Failed to initialize SLAM pipeline");
    return -1;
  }

  omni_slam::Mapper mapper;
  stereo_vio.set_mapper_input_queue(mapper.input_queue());

  rerun::RecordingStream rec("stereo_slam");
  ...
  while (loader.has_camera_data()) {
    if (stereo_vio.fetch_result(result)
  ...
  stereo_vio.shutdown();
  mapper.shutdown();
  LogI("SLAM application finished");
